<template>
    <div class="productbackgroud">
        <el-form label-width="80px" class=" systext">
            <el-form-item label="标题:" style="text-align: left">
                <el-input v-model="textInfo.title" style="width: 45%" readonly="readonly"></el-input>
            </el-form-item>
            <el-form-item label="正文内容:">
                <el-input type="textarea" :rows="10" v-model="textInfo.text" readonly="readonly" style="width: 45%"></el-input>
            </el-form-item>
            <el-form-item label="背景图片:">
                <span v-if="textInfo.backgroudurl!=null">
                    <img style="width: 341.5px; height:177.25px;border-radius: 6px;" :src="textInfo.backgroudurl">
                </span>
                <span v-else style="color: black">暂无背景图片</span>
            </el-form-item>
            <el-form-item label="标题图片:">
                <span v-if="textInfo.titleurl!=null">
                    <img style="width: 346px; height: 67px;border-radius: 6px;" :src="textInfo.titleurl">
                </span>
        ...
        >
            ...
            <el-form :model="formData" :rules="rules" ref="ruleForm" label-width="110px" class="demo-ruleForm">
                ...
                <el-form-item label="正文内容:" prop="text">
                    <el-input type="textarea" :rows="10" maxlength="500" v-model="formData.text"></el-input>
                </el-form-item>
                <el-form-item label="背景图片" prop="backgroudurl" class="textbackgroudurl">
                    <el-upload
                    ...
                    >
                        <img v-if="formData.backgroudurl" :src="formData.backgroudurl"  >
                        <i v-else class="el-icon-plus avatar-uploader-icon" style="font-size: 26px;line-height: 6;"></i>
                    </el-upload>
                    <span>注：上传图片尺寸宽为为2732像素</span>
                </el-form-item>
                <el-form-item label="标题图片" prop="titleurl" class="texttitleurl">
                    <el-upload
                        class="avatar-uploader"
                        :action="uploaduri"
                        :on-success="handleTitleUrlSuccess"
                        :before-upload="beforeAvatarUpload"
                        accept=".png, .jpg, .gif, .jpeg, .bmp"
                        :show-file-list="false"
                    >
                        <img v-if="formData.titleurl" :src="formData.titleurl"  >
...
<script>
    import {getSysTextInfo,textEdit} from "../../api/wesetting/wesetting";
        ...
        methods: {
            ...
            handleBackgroudUrlSuccess(res) {
                ...
                if(res.code==0){
                    ...
                    this.formData.backgroudurl=res.data;
                }
            },
            /*标题图片上传*/
            handleTitleUrlSuccess(res) {
                console.info(res)
                if(res.code==0){
                    alert(res.message)
                }else if(res.code==9){
                    ...
                    this.formData.titleurl=res.data;
                }
            },
            /*图片上传前操作*/
            ...
            editSumbit(){
                ...
                this.formData=  JSON.parse(JSON.stringify(this.textInfo))
            ...
            formSumbit() {
                ...
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        ...
                        //提交操作
                        if(this.formData.textid==null){
                            delete this.formData.textid;
                        }
                        textEdit(this.formData).then(response =>{
                            if (response.code) {
                                this.$message.error(response.message);
...
<style type="text/scss" lang="scss">
    ...
    .textbackgroudurl .el-upload{
        width: 341.5px;
        height:  177.25px;
    }
    .textbackgroudurl img{
        width: 341.5px;
        height:  177.25px;
    }
    .texttitleurl .el-upload{
        width: 346px;
        height: 67px;
    }
    .texttitleurl img{
