<template>
  <div style="padding: 10px;">
    <el-button-group>
      ...
      <el-popover placement="top" title="上传" width="100%" v-model="uploadVisible" trigger="click">
        <el-upload
        ...
        >
          ...
          >只能上传{{ uploadData.exts }}文件，且不超过{{ uploadData.size | renderSize }}</div>
        </el-upload>
    ...
    <div v-if="isIcon" class="icon-lists">
      ...
      <template v-if="lists.length <= 0">
        <div class="icon-list-empty">暂无数据</div>
      </template>
    </div>
